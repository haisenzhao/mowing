...
namespace hpcg {
	...
	{
		//generate 2D Segment Delaunay Graphs
		...
		//generate medial axis and voronoi graph
		...
		//Detect minimal points on medial axis
		//Compute cutting points of the minimal points
		for (int i = 0; i < medial_axis_points.size(); i++)
		{
			if (medial_axis_points_degree[i] == 2 && !medial_axis_points_used[i])
			{
				...
				if (medial_axis_points_radius[i] < medial_axis_points_radius[last_index] && medial_axis_points_radius[i] < medial_axis_points_radius[next_index])
				{
					...
					ComputeCuttingPoints(medial_axis_points[i], medial_axis_points_radius[i], cutting_points);
				}
			}
		...
		DecomposeSubregions(cutting_points, contour, polygons);

		std::vector<Vector2d>().swap(contour);
	}
	
	void ToolpathGenerator::ToolpathGeneration()
	{
		...
		for (int i = 0; i < cutting_points.size(); i = i + 2)
		{
			...
			{
				if (MinimalDistance(polygons[j], cutting_points[i]) < 0.000001)
				{
			...
			entry_exit_points.push_back(ComputeEntryExitPoint(polygons[connected_graph[connected_graph.size() - 2]], cutting_points[i]));
			entry_exit_points.push_back(ComputeEntryExitPoint(polygons[connected_graph[connected_graph.size() - 2]], cutting_points[i + 1]));
			entry_exit_points.push_back(ComputeEntryExitPoint(polygons[connected_graph[connected_graph.size() - 1]], cutting_points[i]));
			entry_exit_points.push_back(ComputeEntryExitPoint(polygons[connected_graph[connected_graph.size() - 1]], cutting_points[i + 1]));
	...
	//Decompose the whole region into sub-regions with the cutting points
	void ToolpathGenerator::DecomposeSubregions(std::vector<Vector2d> &cut_points, std::vector<Vector2d> &contour, std::vector<std::vector<Vector2d>> &polygons)
	{
	...
	void ToolpathGenerator::ComputeCuttingPoints(Vector2d &v, double distace, std::vector<Vector2d> &vecs)
	{
		std::vector<Vector2d> contour;
	...
	Vector2d ToolpathGenerator::ComputeEntryExitPoint(std::vector<Vector2d> &contour, Vector2d &v)
	{
		...
		GenerateOffset(contour, toolpath_size/2.0, offset);
		Vector2d vt;
	...
	void ToolpathGenerator::GenerateOffsetsForAllPolygons()
	{
		for (int i = 0; i < polygons.size(); i++)
		{
			...
			do
			{
				...
				GenerateOffset(polygons[i], lOffset, offset);
